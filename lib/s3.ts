...
function resolveAwsConfig(): AwsConfig {
    ...
    const accessKeyIdRaw = process.env.AWS_ACCESS_KEY_ID_ID ?? process.env.AWS_ACCESS_KEY_ID ?? "";
    const secretAccessKeyRaw = process.env.AWS_SECRET_ACCESS_KEY ?? process.env.AWS_SECRET_ACCESS_KEY ?? "";
    ...
    if (!accessKeyIdRaw.trim() || !secretAccessKeyRaw.trim()) {
        missing.push("AWS_ACCESS_KEY_ID_ID/AWS_SECRET_ACCESS_KEY or AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY");
    }
